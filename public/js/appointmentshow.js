...
fetch(`/patient/appointment?token=${token}&page=${page}`).then((response)=>{
    response.json().then((data)=>{
        for(let i=0;i<data.Appointments.length;i++){
           ...
           <td><a href="/appointment/delete?token=${token}&id=${data.Appointments[i]._id}"><span class="material-icons">remove_circle_outline</span></a><a href="/appointment/edit?token=${token}&id=${data.Appointments[i]._id}"><span class="material-icons">edit</span></a></td>
           </tr>`) 
        }
...
function next(){
    ...
    location.href=`/admin/appointments?&token=${token}&page=${page1}`
}
function back(){
    var previouspage=parseInt(pageNumber,10)-1
    location.href=`/admin/appointments?&token=${token}&page=${previouspage}`
}