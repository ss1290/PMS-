...
fetch(`/admit/show?token=${token}&page=${page}`).then((response)=>{
    response.json().then((data)=>{
        for(let i=0;i<data.Admits.length;i++){
           ...
           <td><a href="/admit/delete?token=${token}&id=${data.Admits[i]._id}"><span class="material-icons">remove_circle_outline</span></a><a href="/admit/edit?token=${token}&id=${data.Admits[i]._id}"><span class="material-icons">edit</span></a></td>
           </tr>`) 
        }
...
function back(){
    ...
    location.href=`/admin/admit?&token=${token}&page=${previouspage}`
}