...
appointmentForm.addEventListener('submit',(e)=>{
    e.preventDefault()
    fetch(`/patient/appointments?token=${token}&name=${patientname.value}&doctor=${doctor.value}&department=${department.value}&date=${date.value}&time=${time.value}&email=${email.value}&gender=${gender.value}&bloodGroup=${bloodGroup.value}&phoneNumber=${phoneNumber.value}&address=${address.value}&age=${age.value}`).then((response)=>{
